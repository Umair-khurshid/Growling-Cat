...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        logger.error("Usage: python crawl.py <URL> [DEPTH_LIMIT] [DOWNLOAD_DELAY] [CONCURRENT_REQUESTS] [JS_RENDERING]")
        sys.exit(1)
    ...
    "EXTENSIONS": {
        "extensions.ProgressExtension": 500,  # path to the class
    },
}

    process = CrawlerProcess(settings=settings)
    try:
        process.crawl(SEOCrawler, start_url=url, js_rendering=js_rendering)
        process.start()
