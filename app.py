...
def style_dataframe(df):
    ...
    styler = df.style.map(style_status_code, subset=['status_code']) \
                     ...
                     .map(
                         lambda x: style_length(x, 120, 158, 20),
...
def display_dashboard(df):
    """Displays the dashboard with metrics."""
    st.write("### 📊 Dashboard")
    total_pages = len(df)
...
def display_faq():
    ...
    with st.expander("Why did my crawl fail or return no results?"):
        st.write(
            """
            Websites can employ anti-scraping measures. If a crawl fails or returns zero pages,
...
def load_and_display_results():
    """Loads the results from the database and displays them."""
    db_file = "growling_cat.db"
    if os.path.exists(db_file):
        try:
            ...
            if all(
                ...
                ['status_code', 'title_length', 'meta_description_length']
            ):
